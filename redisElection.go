...
func (receiver *redisElection) Election(key string, fn func()) {
	for {
		cmd := receiver.rdb.SetNX(fs.Context, key, core.AppId, 20*time.Second)
		// 拿到锁了
...
func (receiver *redisElection) leaseRenewal(key string, ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			receiver.rdb.Del(fs.Context, key).Result()
			flog.Info(key + "退出")
			return
		case <-time.After(10 * time.Second):
			for {
				result, err := receiver.rdb.Expire(ctx, key, 20*time.Second).Result()
				if result {
					flog.Info(key + "续约成功")
					break
				}
